...
export class GameManager extends Component {
    ...
    upgradeBoat() {
        if (this.money < this.boatPrice) return;

        if (this.btnClickStore) {
            this.onHandlerGotoStore();
        }
        AudioManager.getInstance().playSoundUpgade();
        this.vfxBoat.active = true;
        this.updateMoney(-this.boatPrice);
        this.btnClickStore = true;
        this.boatPrice = 150;
        this.labelBtnUpgrade.string = '150';
        if (this.money < 150) {
            this.labelBtnUpgrade.color = color(255, 0, 0);
        }
        else {
